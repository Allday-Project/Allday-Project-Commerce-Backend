...
public class EventOrderController {
    ...
    /**
     * v8 - Redisson Blocking + Watchdog + 비관락
     */
    @PostMapping("/v8/products/{productId}/orders")
    public ResponseEntity<ApiResponse<EventOrderResponse>> v8(
            @PathVariable Long productId,
            @RequestParam Long userId
    ) {
        return ok(eventOrderFacade.createEventOrderWithRedissonLockAopBlockingWatchdogWithPessimisticLock(productId, userId));
    }
