...
class EventOrderConcurrencyTest {
    ...
    void createEventOrderWithoutLock_concurrency_test() throws Exception {

        // given
        prepareStock(TEST_TICKET_STOCK);
        ...
        Product product = productRepository.findById(TEST_TICKET_PRODUCT_ID).orElseThrow();
    ...
    private void prepareStock(int stock) {
        Product product = productQueryService.getByProductId(TEST_TICKET_PRODUCT_ID);

        if (product.getStock() > stock) {
            product.decreaseStock(product.getStock() - stock);
        }

        if (product.getStock() < stock) {
            product.increaseStock(stock - product.getStock());
        }
     ...
     * 🔥 결과 출력
     */
    private void printResult(ConcurrencyTestResult result) {
